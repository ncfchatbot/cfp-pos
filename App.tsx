      ...
      <div className="p-4 md:p-6 h-full overflow-y-auto bg-slate-50/50">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            {recentSales.length === 0 && <div className="py-10 text-center text-slate-300 text-xs italic">No activity yet.</div>}
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex h-full flex-col md:flex-row overflow-hidden bg-slate-50/50">
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 p-4 overflow-y-auto"><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">{filteredProducts.map(p => (<button key={p.id} onClick={() => addToCart(p)} className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 hover:border-sky-300 transition-all text-left flex flex-col h-full group"><div className={`w-full aspect-square rounded-2xl ${p.color} mb-4 flex items-center justify-center text-2xl font-bold overflow-hidden group-hover:scale-105 transition-transform`}>{p.imageUrl ? <img src={p.imageUrl} className="w-full h-full object-cover" /> : p.name.charAt(0)}</div><h3 className="font-bold text-slate-800 text-sm line-clamp-2 mb-1 flex-1">{p.name}</h3><div className="flex justify-between items-center mt-2"><p className="text-sky-600 font-bold text-sm">{formatCurrency(p.price, language)}</p><span className="text-[10px] text-slate-300 font-mono">{p.stock}</span></div></button>))}{filteredProducts.length === 0 && <div className="col-span-full py-20 text-center text-slate-300">No products found.</div>}</div></div>
        </div>
  ...
  const renderReports = () => {
    ...
    return (
      <div className="p-4 md:p-6 h-full overflow-y-auto bg-slate-50/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8"><div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm"><p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-2">Estimated Net Profit</p><h3 className="text-4xl font-bold text-sky-600 tracking-tighter mb-4">{formatCurrency(totalProfit, language)}</h3></div><div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-center"><p className="text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sales Trend (Sample)</p></div></div>
      </div>
    );
  };
      ...
      <div className="max-w-4xl space-y-6">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div className="flex flex-col md:flex-row gap-10">
               <div className="flex flex-col items-center gap-4"><div onClick={() => logoInputRef.current?.click()} className="w-36 h-36 rounded-3xl bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer overflow-hidden group hover:border-sky-400 transition-all">{storeProfile.logoUrl ? <img src={storeProfile.logoUrl} className="w-full h-full object-cover" /> : <div className="text-slate-300 flex flex-col items-center gap-2"><ImagePlus size={32}/><span className="text-[10px] font-bold uppercase">Logo</span></div>}</div><input type="file" ref={logoInputRef} className="hidden" accept="image/*" onChange={e => { const f = e.target.files?.[0]; if (f) { const r = new FileReader(); r.onloadend = () => setStoreProfile(prev => ({ ...prev, logoUrl: r.result as string })); r.readAsDataURL(f); } }} /></div>
               <div className="flex-1 space-y-5"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Shop Name</label><input value={storeProfile.name} onChange={e => setStoreProfile({...storeProfile, name: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Phone</label><input value={storeProfile.phone} onChange={e => setStoreProfile({...storeProfile, phone: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold outline-none"/></div></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Address</label><textarea value={storeProfile.address} onChange={e => setStoreProfile({...storeProfile, address: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold h-24 outline-none resize-none"/></div><button onClick={()=>alert('Saved')} className="flex items-center gap-2 px-6 py-3 bg-sky-600 text-white rounded-2xl font-bold text-xs shadow-lg hover:scale-95 transition-all"><Save size={16} /> Save Changes</button></div>
            </div>
          </div>
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100"><h3 className="font-bold text-slate-400 mb-6 flex items-center gap-2 text-[10px] uppercase tracking-widest"><DatabaseBackup size={16}/> Advanced Settings</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><button className="p-6 bg-slate-50 rounded-2xl hover:bg-sky-50 border border-slate-100 flex flex-col items-center gap-3 transition-all"><Download className="text-sky-600"/><span className="text-[10px] font-bold uppercase tracking-widest">Backup</span></button><button onClick={() => csvInputRef.current?.click()} className="p-6 bg-slate-50 rounded-2xl hover:bg-sky-50 border border-slate-100 flex flex-col items-center gap-3 transition-all"><Upload className="text-sky-600"/><span className="text-[10px] font-bold uppercase tracking-widest">Import CSV</span></button><button onClick={() => { if(confirm('Factory Reset?')) { localStorage.clear(); window.location.reload(); } }} className="p-6 bg-red-50/50 rounded-2xl hover:bg-red-50 border border-red-100 flex flex-col items-center gap-3 transition-all text-red-500"><Trash2 /><span className="text-[10px] font-bold uppercase tracking-widest">Clear Data</span></button></div></div>
      </div>
    </div>
  );
    ...
    <div className={`flex h-screen bg-slate-100 text-slate-900 font-sans ${language === 'th' ? 'font-thai' : ''}`}>
      ...
      {isProductModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl"><h3 className="text-xl font-bold mb-6 text-slate-800">{editingProduct ? 'Edit Product' : 'Create Product'}</h3><form onSubmit={handleSaveProduct} className="space-y-5"><div className="grid grid-cols-2 gap-4"><div className="col-span-2 space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Name</label><input name="name" required defaultValue={editingProduct?.name} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">SKU</label><input name="code" required defaultValue={editingProduct?.code} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Cat</label><input name="category" required defaultValue={editingProduct?.category} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Cost</label><input name="cost" type="number" required defaultValue={editingProduct?.cost} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Price</label><input name="price" type="number" required defaultValue={editingProduct?.price} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none text-sky-600"/></div><div className="col-span-2 space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Stock</label><input name="stock" type="number" required defaultValue={editingProduct?.stock} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div></div><div className="flex gap-3 pt-4"><button type="button" onClick={()=>setIsProductModalOpen(false)} className="flex-1 py-4 border-2 border-slate-100 rounded-2xl font-bold text-slate-400 text-xs uppercase tracking-widest">Discard</button><button type="submit" className="flex-2 py-4 bg-sky-600 text-white rounded-2xl font-bold shadow-lg px-10 text-xs uppercase tracking-widest">Save</button></div></form></div>
        </div>
      )}

      {isPromotionModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl p-8 shadow-2xl overflow-y-auto max-h-[90vh]"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-slate-800">{editingPromotion ? 'Edit Promotion' : 'New Promotion'}</h3><button onClick={() => setIsPromotionModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X/></button></div><form onSubmit={handleSavePromotion} className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t.promo_name}</label><input name="name" required defaultValue={editingPromotion?.name} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t.promo_type}</label><select name="type" value={selectedPromoType} onChange={(e) => setSelectedPromoType(e.target.value as PromotionType)} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none cursor-pointer"><option value="buy_x_get_y">{t.promo_buy_get}</option><option value="tiered_price">{t.promo_tiered}</option></select></div></div><div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Target Product SKUs (comma separated)</label><input name="skus" placeholder="e.g. COFFEE-01, TEA-02" required defaultValue={editingPromotion?.targetSkus.join(', ')} className="w-full p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none"/></div>
              {selectedPromoType === 'buy_x_get_y' ? (<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 bg-sky-50 rounded-3xl border border-sky-100"><div className="space-y-1"><label className="text-[10px] font-bold text-sky-600 uppercase tracking-widest">Buy Qty</label><input name="requiredQty" type="number" defaultValue={editingPromotion?.requiredQty} className="w-full p-3 bg-white border border-sky-100 rounded-xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-sky-600 uppercase tracking-widest">Free Qty</label><input name="freeQty" type="number" defaultValue={editingPromotion?.freeQty} className="w-full p-3 bg-white border border-sky-100 rounded-xl text-sm font-bold outline-none"/></div><div className="space-y-1"><label className="text-[10px] font-bold text-sky-600 uppercase tracking-widest">Free SKU (optional)</label><input name="freeSku" defaultValue={editingPromotion?.freeSku} className="w-full p-3 bg-white border border-sky-100 rounded-xl text-sm font-bold outline-none"/></div></div>) : (
                <div className="p-5 bg-amber-50 rounded-3xl border border-amber-100 space-y-4"><div className="flex items-center justify-between mb-2"><h4 className="text-xs font-bold text-amber-700 uppercase tracking-widest">7-Step Pricing Tiers</h4></div><div className="grid grid-cols-8 gap-3 items-center text-[9px] font-bold text-amber-600 uppercase tracking-tighter mb-2"><div className="col-span-1">Step</div><div className="col-span-3">Minimum Qty</div><div className="col-span-4">Sale Price (LAK)</div></div>
                  {[1, 2, 3, 4, 5, 6, 7].map(step => (<div key={step} className="grid grid-cols-8 gap-3 items-center"><div className="col-span-1 text-center font-bold text-amber-700 text-xs">{step}</div><div className="col-span-3"><input name={`tier_qty_${step}`} type="number" placeholder="Qty" defaultValue={editingPromotion?.tiers?.[step-1]?.minQty} className="w-full p-2.5 bg-white border border-amber-100 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-amber-500"/></div><div className="col-span-4"><input name={`tier_price_${step}`} type="number" placeholder="Price" defaultValue={editingPromotion?.tiers?.[step-1]?.price} className="w-full p-2.5 bg-white border border-amber-100 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-amber-500 text-sky-700"/></div></div>))}
                </div>
              )}<div className="flex gap-3 pt-4"><button type="button" onClick={()=>setIsPromotionModalOpen(false)} className="flex-1 py-4 border-2 border-slate-100 rounded-2xl font-bold text-slate-400 text-xs uppercase tracking-widest transition-all hover:bg-slate-50">Discard</button><button type="submit" className="flex-2 py-4 bg-sky-600 text-white rounded-2xl font-bold shadow-lg px-12 text-xs uppercase tracking-widest transition-all hover:bg-sky-700">Save Promotion</button></div></form></div>
        </div>
      )}

      {showReceipt && currentOrder && (
        <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-4 backdrop-blur-xl">
            <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden flex flex-col shadow-2xl"><div id="receipt-content" className="p-10 text-slate-800 bg-white font-mono text-[11px] leading-relaxed"><div className="text-center border-b-2 border-dashed border-slate-200 pb-6 mb-6">{storeProfile.logoUrl ? <img src={storeProfile.logoUrl} className="w-16 h-16 mx-auto mb-4 object-contain rounded-xl" /> : <div className="w-12 h-12 bg-sky-50 rounded-xl mx-auto mb-4 flex items-center justify-center text-sky-600 font-bold">CP</div>}<h2 className="text-lg font-bold uppercase tracking-widest">{storeProfile.name}</h2><p className="text-[9px] text-slate-400 max-w-[200px] mx-auto mt-2 leading-tight uppercase">{storeProfile.address}</p></div><div className="space-y-3 mb-6"><div className="flex justify-between text-[9px] text-slate-400 uppercase font-bold tracking-widest"><span>Item</span><span>Price</span></div>{currentOrder.items.map((it, i) => (<div key={i} className="flex justify-between items-start gap-4"><span className="flex-1">{it.name} <span className="text-slate-400">x{it.quantity}</span></span><span className="whitespace-nowrap font-bold">{formatCurrency(it.price * it.quantity, language)}</span></div>))}</div><div className="border-t-2 border-dashed border-slate-200 mt-6 pt-6 space-y-2"><div className="flex justify-between text-slate-400 uppercase text-[9px] font-bold"><span>Subtotal:</span><span>{formatCurrency(currentOrder.subtotal || currentOrder.total, language)}</span></div><div className="flex justify-between text-lg font-bold text-slate-900 pt-2"><span className="uppercase tracking-widest">Total:</span><span>{formatCurrency(currentOrder.total, language)}</span></div></div><div className="text-center mt-10 text-[9px] text-slate-300 font-bold uppercase tracking-[0.3em]">Thank You • ขอบพระคุณ</div></div><div className="p-6 bg-slate-50 border-t flex gap-3"><button onClick={()=>setShowReceipt(false)} className="flex-1 py-4 bg-white border border-slate-200 rounded-2xl font-bold text-slate-400 text-xs uppercase tracking-widest">Close</button><button onClick={()=>window.print()} className="flex-1 py-4 bg-sky-600 text-white rounded-2xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-sky-100">Print</button></div></div>
        </div>
      )}
    </div>
  );
